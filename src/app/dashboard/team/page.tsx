    ...
    <div className="-mx-4 sm:-mx-6 md:-mx-8 lg:-mx-10 -mt-5 sm:-mt-6 md:-mt-8">
      ...
      <div className="relative overflow-hidden" style={{ height: '180px' }}>
        ...
        <div className="absolute bottom-6 left-6 lg:left-10 z-10">
          <p className="text-[10px] uppercase tracking-[0.2em] text-white/20 mb-2">Organizzazione</p>
        ...
        <div className="absolute bottom-6 right-6 lg:right-10 z-10">
          <button
            ...
            className="px-4 py-2 text-[9px] uppercase tracking-[0.15em] bg-white text-black flex items-center gap-1.5 font-medium"
          >
      ...
      <div className="px-6 lg:px-10 py-8">
        ...
        <div className="grid grid-cols-3 gap-px bg-white/[0.06] mb-8">
          ...
          <div className="bg-[#0A0A0A] p-5">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/25 mb-2">Admin</p>
            <p className="text-3xl font-bold text-white" style={{ fontFamily: 'var(--font-heading)' }}>
              {staffUsers.filter(u => u.role === 'ceo').length}
            </p>
          </div>
          <div className="bg-[#0A0A0A] p-5">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/25 mb-2">Dipendenti</p>
            <p className="text-3xl font-bold text-white" style={{ fontFamily: 'var(--font-heading)' }}>
            ...
            <div key={user.id} className="flex items-center gap-6 px-6 py-5 border-b border-white/[0.03] last:border-b-0 hover:bg-white/[0.015] transition-colors group">
              ...
              <div className="hidden md:flex flex-wrap gap-1 shrink-0 max-w-[200px]">
                {user.permissions.includes('all') ? (
                  <span className="px-2 py-0.5 text-[7px] uppercase tracking-[0.15em] bg-white/[0.04] text-white/30 flex items-center gap-1">
                    <ShieldCheck size={8} /> Tutti i permessi
                  </span>
                ) : (
                  user.permissions.slice(0, 3).map(p => (
                    <span key={p} className="px-2 py-0.5 text-[7px] uppercase tracking-[0.15em] bg-white/[0.03] text-white/15">
              ...
              <div className="flex items-center gap-px opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <button className="w-7 h-7 bg-white/[0.04] flex items-center justify-center text-white/15 hover:text-white/40 transition-colors">
                  <Edit3 size={10} />
                </button>
                {user.role !== 'ceo' && (
                  <>
                    ...
                    <button className="w-7 h-7 bg-white/[0.04] flex items-center justify-center text-white/15 hover:text-red-400/40 transition-colors">
                      <Trash2 size={10} />
                    </button>
                  </>
                )}
        ...
        <div className="mt-8 border border-white/[0.06]">
          <div className="px-5 py-3 border-b border-white/[0.06]">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/30">Matrice permessi</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left px-4 py-3 text-[8px] uppercase tracking-[0.15em] text-white/20 w-40">Membro</th>
                  {['athletes', 'calendar', 'training', 'measurements', 'payments', 'analytics', 'team'].map(p => (
                    <th key={p} className="text-center px-3 py-3 text-[7px] uppercase tracking-[0.15em] text-white/15">
                      {permissionLabels[p]}
                    </th>
                  ))}
                </tr>
        ...
        <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-6" onClick={() => setShowNewModal(false)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-[#111] border border-white/[0.08] w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06]">
              <h3 className="text-[10px] uppercase tracking-[0.2em] text-white/40">Nuovo membro</h3>
              <button onClick={() => setShowNewModal(false)} className="text-white/20 hover:text-white transition-colors"><X size={16} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-[9px] uppercase tracking-[0.2em] text-white/20 mb-1.5 block">Nome</label><input type="text" placeholder="Nome" className="nike-input" /></div>
                <div><label className="text-[9px] uppercase tracking-[0.2em] text-white/20 mb-1.5 block">Cognome</label><input type="text" placeholder="Cognome" className="nike-input" /></div>
              </div>
              <div><label className="text-[9px] uppercase tracking-[0.2em] text-white/20 mb-1.5 block">Email</label><input type="email" placeholder="email@grperform.com" className="nike-input" /></div>
              <div><label className="text-[9px] uppercase tracking-[0.2em] text-white/20 mb-1.5 block">Password temporanea</label><input type="password" placeholder="••••••" className="nike-input" /></div>
              <div><label className="text-[9px] uppercase tracking-[0.2em] text-white/20 mb-1.5 block">Ruolo</label>
              ...
              <div>
                <label className="text-[9px] uppercase tracking-[0.2em] text-white/20 mb-2 block">Permessi</label>
                <div className="grid grid-cols-2 gap-2">
                  {['athletes', 'calendar', 'training', 'measurements', 'payments', 'analytics', 'team'].map(p => (
                    <label key={p} className="flex items-center gap-2 cursor-pointer group">
                      ...
                      <span className="text-[10px] text-white/25 group-hover:text-white/40 transition-colors">{permissionLabels[p]}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="px-6 py-5 border-t border-white/[0.06] flex items-center justify-end gap-3">
              <button onClick={() => setShowNewModal(false)} className="px-4 py-2 text-[9px] uppercase tracking-[0.15em] text-white/20 hover:text-white/50 transition-colors">Annulla</button>
              <button className="px-5 py-2.5 text-[9px] uppercase tracking-[0.15em] bg-white text-black font-medium">Crea membro</button>
            </div>
          </div>
        </div>
      )}
